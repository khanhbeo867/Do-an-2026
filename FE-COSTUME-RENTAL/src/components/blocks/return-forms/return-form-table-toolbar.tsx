...
const ReturnFormTableToolbar: React.FC<{
  table: Table<IReturnForm>
  event$: EventEmitter<Record<string, unknown>>
}> = ({ table }) => {
  const isMobile = useMediaQuery('(max-width: 767px)')
  const { data, refetch, isRefetching } = useGetReturnFormsQuery()
  const router = useRouter()
  const isFiltered = table.getState().columnFilters.length > 0 || table.getState().globalFilter

  const borrowerOptions = useMemo(() => {
    return Array.from(
      new Set((data ?? []).map((item) => item.loan_form?.borrower_name ?? item.returnee_name).filter(Boolean))
    ).map((name) => ({
      label: name,
      value: name,
    }))
...
export default ReturnFormTableToolbar
